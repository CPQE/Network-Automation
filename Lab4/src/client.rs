...
use crate::utilities::{BulletinMessage, deserialize_messages};

const MAX_PACKET_SIZE: usize = 512; // response from server can be up to 5 messages, needs more headroom than client sends
...
pub fn run_client(server_ip: &str, port: u16, input_file: Option<&str>) -> std::io::Result<()> {
    ...
    match input_file {
        Some(path) => {
            // file mode: read all lines from file, send each as a separate message
        ...
        None => {
            // stdin mode: prompt user repeatedly until they quit
            let stdin = io::stdin();
            loop {
                ...
                let line = line.trim();
                if line.eq_ignore_ascii_case("quit") || line.is_empty() {
                    break;
                }
                if line.len() > 250 {
